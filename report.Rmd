...
```{r Load data}
counts <- load_n_trim('data/verse_counts.tsv')
head(counts)
```
...
group
edger_res <- run_edger(counts, group)
cat("edger without padj:\n")
head(edger_res, 3)
cat("edger _with_ padj:\n")
edger_res$padj <- p.adjust(edger_res$PValue, method="BH")
head(edger_res, 3)
```
...
group <- factor(rep(c(1,2), each=2))
group
limma_res <- run_limma(counts, design, group)
cat("Limma results:\n")
head(limma_res)
```
...
str(limma_res)
o <- order(deseq_res$pvalue)
deseq <- head(deseq_res[o,], 1000)
o <- order(edger_res$PValue)
edger <- head(edger_res[o,], 1000)
o <- order(limma_res$P.Value)
limma <- head(limma_res[o,], 1000)
...
x <- list(EdgeR = row.names(edger), DESeq2 = row.names(deseq), Limma = row.names(limma))
ggVennDiagram(x, color = "black") + 
  scale_fill_gradient(low = "#000051", high = "#4fc3f7") +
...
combined_vals <- combine_pval(deseq, edger, limma)
as.data.frame(combined_vals) %>%
ggplot(aes(pval)) + 
    geom_histogram(bins=50, color='black', fill='#00363a') + 
...
volcano_data <- create_facets(deseq, edger, limma)
plot <- theme_plot(volcano_data)
plot
```
...
row.names(design) <- c("vP0_1", "vP0_2", "vAd_1", "vAd_2")
group <- factor(rep(c(1,2), each=2))
limma_trend_res <- voomless(counts, design, group)
o <- order(limma_trend_res$P.Value)
limma_trend <- head(limma_trend_res[o,], 1000)
x <- list(EdgeR = row.names(edger), DESeq2 = row.names(deseq), Voom = row.names(limma), Limma = row.names(limma_trend))
ggVennDiagram(x, color = "black") +
  scale_fill_gradient(low = "#000051", high = "#4fc3f7") +
  ggtitle('Top 1000 genes from FOUR diff. exp. R packages')

combined_vals <- combine_pval2(deseq, edger, limma, limma_trend)
as.data.frame(combined_vals) %>%
ggplot(aes(pval)) + 
    geom_histogram(bins=50, color='black', fill='#00363a') + 
    ggtitle('Distributions of p-values for 1,000 genes: day0 vs adult') +
  facet_wrap(~package, nrow = 4) +
  theme(panel.background = element_rect(fill='white'), panel.border = element_rect(colour = "black", fill=NA, size=1))

volcano_data <- create_facets2(deseq, edger, limma, limma_trend)
plot <- theme_plot2(volcano_data)
plot
```
